...
import { CellPropertiesFields } from './CellPropertiesFields'

const FOCUSABLE =
  'button:not([disabled]):not([tabindex="-1"]), input:not([disabled]), select:not([disabled]), textarea:not([disabled])'

export type CellPropertiesDialogProps = {
  open: boolean
  value: CellPropertiesApply
  disabled?: boolean
  onApply: (draft: CellPropertiesApply) => void
  onClose: () => void
}
      ...
      >
        <div className={styles.headerRow}>
          <h2 className={styles.header} id={titleId}>
            {t('cellPropertiesDialogTitle')}
          </h2>
          <button
            type="button"
            className={styles.close}
            aria-label={t('cellPropertiesDialogCloseAria')}
            onClick={onClose}
          >
            <CloseIcon />
          </button>
        </div>
        <CellPropertiesFields value={draft} disabled={disabled} onChange={setDraft} />
